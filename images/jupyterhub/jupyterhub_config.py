...
#  Example uses:
#
#  - You can set `notebook_dir` to `/` and `default_url` to `/tree/home/{username}` to allow
#    people to navigate the whole filesystem from their notebook server, but still start in
#    their home directory.
#  - Start with `/notebooks` instead of `/tree` if `default_url` points to a notebook instead
#    of a directory.
#  - You can set this to `/lab` to have JupyterLab start by default, rather than Jupyter
#    Notebook.
#  Default: ''
...
#            'LANG', 'LC_ALL', 'JUPYTERHUB_SINGLEUSER_APP']
...
#    - Variables to establish contact between the single-user notebook and the hub (such as
